class ClassListsController < ApplicationController
  ...
  def index
	...
	@my_reserve_class_lists = ClassStatus.where(student_id: current_student.id, status: "예약")
	  @class_statusess = ClassStatus.all
  end
  ...
  private
    ...
    def class_list_params
      params.require(:class_list).permit(:c_name, :credit, :classify, :sub_type, :sub_num, :target_grade, :limit_num, :lecture_time, :department_id, :professor_id)
    end
end
